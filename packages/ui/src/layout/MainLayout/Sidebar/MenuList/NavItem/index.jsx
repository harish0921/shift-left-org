...
const NavItem = ({ item, level, navType, onClick, onUploadFile }) => {
    ...
    useEffect(() => {
        if (navType === 'MENU') {
            const currentPathParts = document.location.pathname.toString().split('/').filter(Boolean)
            const itemPathParts = item.url?.split('/').filter(Boolean) ?? []
            const matchesById = currentPathParts.includes(item.id)
            const matchesByPath = itemPathParts.length > 0 && currentPathParts.some((part) => itemPathParts.includes(part))
