...
export class App {
    ...
    async initDatabase() {
        ...
        try {
            ...
            logger.info('ðŸ“¦ [server]: Data Source initialized successfully')

            // Run Migrations Scripts
            await this.AppDataSource.runMigrations({ transaction: 'each' })
            logger.info('ðŸ”„ [server]: Database migrations completed successfully')

            // Initialize Identity Manager
            this.identityManager = await IdentityManager.getInstance()
            logger.info('ðŸ” [server]: Identity Manager initialized successfully')

            // Initialize nodes pool
            this.nodesPool = new NodesPool()
            await this.nodesPool.initialize()
            logger.info('ðŸ”§ [server]: Nodes pool initialized successfully')

            // Initialize abort controllers pool
            this.abortControllerPool = new AbortControllerPool()
            logger.info('â¹ï¸ [server]: Abort controllers pool initialized successfully')

            // Initialize encryption key
            await getEncryptionKey()
            logger.info('ðŸ”‘ [server]: Encryption key initialized successfully')

            // Initialize auth secrets (env â†’ AWS Secrets Manager â†’ filesystem)
            await initAuthSecrets()
            logger.info('ðŸ” [server]: Auth initialized successfully')

            // Initialize Rate Limit
            this.rateLimiterManager = RateLimiterManager.getInstance()
            await this.rateLimiterManager.initializeRateLimiters(await getDataSource().getRepository(ChatFlow).find())
            logger.info('ðŸš¦ [server]: Rate limiters initialized successfully')

            // Initialize cache pool
            this.cachePool = new CachePool()
            logger.info('ðŸ’¾ [server]: Cache pool initialized successfully')

            // Initialize usage cache manager
            this.usageCacheManager = await UsageCacheManager.getInstance()
            logger.info('ðŸ“Š [server]: Usage cache manager initialized successfully')

            // Initialize telemetry
            this.telemetry = new Telemetry()
            logger.info('ðŸ“ˆ [server]: Telemetry initialized successfully')

            // Initialize SSE Streamer
            this.sseStreamer = new SSEStreamer()
            logger.info('ðŸŒŠ [server]: SSE Streamer initialized successfully')
            ...
            if (process.env.MODE === MODE.QUEUE) {
                ...
                logger.info('âœ… [Queue]: All queues setup successfully')

                this.redisSubscriber = new RedisEventSubscriber(this.sseStreamer)
                await this.redisSubscriber.connect()
                logger.info('ðŸ”— [server]: Redis event subscriber connected successfully')
            }

            logger.info('ðŸŽ‰ [server]: All initialization steps completed successfully!')
        } catch (error) {
            logger.error('âŒ [server]: Error during Data Source initialization:', error)
    ...
    async config() {
        ...
        if (process.env.ENABLE_METRICS === 'true') {
            ...
            if (this.metricsProvider) {
                ...
                logger.info(`ðŸ“Š [server]: Metrics Provider [${this.metricsProvider.getName()}] has been initialized!`)
            } else {
                logger.error(
                    "âŒ [server]: Metrics collection is enabled, but failed to initialize provider (valid values are 'prometheus' or 'open_telemetry'."
                )
            }
        }
    ...
    async stopApp() {
        try {
            ...
            logger.error(`âŒ[server]: Flowise Server shut down error: ${e}`)
        }
    }
}
...
export async function start(): Promise<void> {
    ...
    server.listen(port, host, () => {
        logger.info(`âš¡ï¸ [server]: Flowise Server is listening at ${host ? 'http://' + host : ''}:${port}`)
    })
}
